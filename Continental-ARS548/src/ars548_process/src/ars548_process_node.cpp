...
void receiveThread()
{
    while(ros::ok())
    {
        udp_io.receiveFromRadar(rec_data,rec_length);
                    //   ROS_INFO_STREAM("Hello, 100!") ;

        if(rec_length>0)
        {
               ROS_INFO_STREAM("Hello, 11!") ;
 std::cout << '1212 ';
...
int main(int argc, char **argv)
{
  ...
  ROS_INFO_STREAM("Hello, ROS10!") ;
    ...
    //udp_io.initUdpMulticastServer("192.168.1.102",42102);
              ROS_INFO_STREAM("Hello, 101!") ;

    udp_io.initUdpUnicastClient("10.13.1.113",42101,42401);
              ROS_INFO_STREAM("Hello, 102!") ;

    std::thread rec_thread = std::thread(receiveThread);  
              ROS_INFO_STREAM("Hello, 103!") ;
