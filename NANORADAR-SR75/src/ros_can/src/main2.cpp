 ...
 uint data[8][8];
     ...
     while(canfd_num)
    {
        ...
        cout << "接收canfd报文num:" <<ReadLenfd<< endl;
         ...
         for (int i = 0; i < ReadLenfd; i++)     //报文数，对应点数
         { 
            if(GET_ID(CanFDObj[i].frame.can_id==0x701))
            {
                ...
                printf("\n");
                for (int j=0; j < CanFDObj[i].frame.len; j++)   //每条报文数据位，对应每个点的信息
                  {
                    data[j/8][j%8]=CanFDObj[i].frame.data[j];
                    //printf(" %02X", CanFDObj[i].frame.data[j]);
                  }
                 //printf("\n");
               /*
                for(int i = 0; i < 8; i++) {   
                ...
                // float VDis=((((0xB3&0x07)*256) +(0xE9))*0.05)-50;
                // cout << "VDis:"<<VDis<<" "<< endl;  
/**/
                float VDis,HDis,RVel,High,MovArr,SigNoiRatio,RDis,thea;
                POINTFD pointfd[300];
                for(int a = 0; a < 8; a++) {   
                    for(int b = 0; b < 8; b++) {  
                       switch (b)
